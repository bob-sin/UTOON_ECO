  ...
  <div class="panel panel-default">
    <div class="panel-heading">
    	...
    	<form class="form-inline" action="${cpath}/logout.do">
		  <div class="form-group">
		    <label>${mvo.memName}님환영합니다.</label>
		  </div>
    ...
    <div id="vfrm" class="panel-body">
    	<table class="table table-bordered table-hover"> <!-- 예쁘게 레이아웃 하는법 class="table"  -->
    		<tr>
    			<td>번호</td>
    			<td>제목</td>
    			<td>작성자</td>
    		...
    		<c:forEach var="vo" items ="${list}">
    			<tr>
    			<td>${vo.idx}</td> <!--QueryString :  ?key=value&key=value (공백절대안됨)  -->
    			<td id="t${vo.idx}"><a href = "javascript:contentView(${vo.idx})">${vo.title}</a></td>
    			<!-- <a> 태그 사용하면 get방식 <form> 사용 post방식 -->
    			<td>${vo.writer}</td>
    ...
    <div class="panel-footer">인공지능 융합서비스 개발자과정(신현범)</div>
  </div>
</div>
